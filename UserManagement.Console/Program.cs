...
Console.WriteLine();
Console.WriteLine($"1. Find user by email (e.g., {EMAIL_LOOKUP})");
Console.WriteLine($"2. Find all users older than a given age (e.g., {AGE_THRESHOLD})");
...
{
    Console.Write("Please enter the email to look up: ");
    var email = Console.ReadLine();
    if (string.IsNullOrWhiteSpace(email))
    {
        Console.WriteLine("Error: No email was entered.");
        return;
    }
...
{
    Console.Write("Please enter the age threshold: ");
    var ageInput = Console.ReadLine();
    if (!int.TryParse(ageInput, out var ageThreshold))
    {
        Console.WriteLine("Error: Invalid age threshold.");
        return;
    }
